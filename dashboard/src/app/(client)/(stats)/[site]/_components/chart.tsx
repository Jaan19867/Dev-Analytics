...
const ChartTooltip = React.memo(({ label, date, period, payload, valueFormatter }: ChartTooltipProps) => {
  ...
  // Memoize dateTimeFormat based on the period
  const dateTimeFormat = useMemo(() => {
    ...
    if (period === PERIODS.HALF || period === PERIODS.YEAR || period === PERIODS.ALL) {
  ...
  // Memoize dateLabel based on dateTimeFormat and period
  const dateLabel = useMemo(() => {
    ...
    if (period === 'halfyear' || period === PERIODS.YEAR || period === PERIODS.ALL) {
  ...
  return (
    <Paper px="md" py="md" withBorder shadow="md" radius="md" bg="dark">
      <Text fw={500} mb="xs" c="white">
        {dateLabel}
      </Text>
